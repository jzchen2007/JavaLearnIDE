...
## 🏆 v2.8.2 更新：修复判题一直说输出是 0 + 题解导出

> v2.8.2 修复判题 bug（编辑内容未被保存导致跑旧模板），并支持把题解导出到指定目录。

- **修复判题输出恒为 0**：编辑器改动不再丢失（dirty 标记修复），判题前自动保存所有未保存内容，测试跑的是最新代码
- **题解导出到本地目录**：设置里可指定「题解保存目录」，判题/标记通过后自动导出为 `题号-题目名.py/java`，方便下次查看
- **菲比判题**：也会先保存再判，并自动导出题解

## 🏆 v2.8.1 更新：修复编辑进度丢失

> v2.8.1 修复严重 bug：判题/切换后未保存的编辑进度消失。

- **切换标签不丢内容**：切换标签时先把当前编辑器内容同步回标签，未保存的修改不再丢失
- **判题前自动保存所有**：判题前保存所有未保存的标签（含 Solution），测试永远跑最新代码
- **重开题目不覆盖**：重新打开题目不再覆盖已写的 Solution 与已添加的测试用例
- **关闭标签自动保存**：关闭有未保存修改的标签时自动写盘
